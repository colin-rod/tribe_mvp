...
const DialogContent: React.FC<DialogContentProps> = ({
  ...
  return (
    <div
      className={`relative z-50 w-full ${maxWidthClasses[maxWidth]} mx-4 bg-white rounded-lg shadow-xl overflow-hidden ${className}`}
      role="dialog"
      aria-modal="true"
    >
      {children}
...
const DialogBody: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  return (
    <div className={`p-6 overflow-y-auto max-h-[calc(100vh-200px)] ${className}`}>
      {children}
    </div>
  )
}
