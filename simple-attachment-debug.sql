...
-- 2. Check for updates with HTML that contains images
SELECT
    id,
    subject,
    ...
    created_at
FROM updates
WHERE rich_content->'html'::text LIKE '%<img%'
  AND created_at > NOW() - INTERVAL '1 day'
ORDER BY created_at DESC;

-- 3. Check for updates that mention images but have no media_urls
SELECT
    id,
    subject,
    content,
...
-- 4. Summary of recent email updates
SELECT
    ...
    COUNT(CASE WHEN rich_content->'html'::text LIKE '%<img%' THEN 1 END) as with_html_images,
